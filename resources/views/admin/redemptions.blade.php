...
@push('scripts')
<script>
function loadRedemptions(){fetch('/web-api/redemptions?per_page=20', { credentials: 'same-origin' }).then(r=>r.json()).then(d=>{const t=document.getElementById('redemptionTable');if(d.data&&d.data.length){t.innerHTML=d.data.map(r=>`<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="px-6 py-3">${r.id}</td><td class="px-6 py-3 font-mono text-xs">${r.code}</td><td class="px-6 py-3">${(r.quota||0).toLocaleString()}</td><td class="px-6 py-3">${r.username||'-'}</td><td class="px-6 py-3"><span class="px-2 py-1 text-xs rounded-full ${r.status===1?'bg-green-100 text-green-700':'bg-gray-100 text-gray-700'}">${r.status===1?'已使用':'未使用'}</span></td><td class="px-6 py-3 text-gray-500">${r.created_at?new Date(r.created_at*1000).toLocaleDateString():'-'}</td><td class="px-6 py-3"><button onclick="deleteCode(${r.id})" class="text-red-600 hover:text-red-800">删除</button></td></tr>`).join('')}else{t.innerHTML='<tr><td colspan="7" class="px-6 py-8 text-center text-gray-500">暂无兑换码</td></tr>'}document.getElementById('pageInfo').textContent=`共 ${d.total||0} 条`})}
