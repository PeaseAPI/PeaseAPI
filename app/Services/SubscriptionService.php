...
class SubscriptionService
{
    ...
    {
        if ($plan->duration_unit === 'day') return $startAt + $plan->duration_value * 86400;
        if ($plan->duration_unit === 'month') return $startAt + $plan->duration_value * 30 * 86400;
        if ($plan->duration_unit === 'year') return $startAt + $plan->duration_value * 365 * 86400;
        if ($plan->duration_unit === 'custom') return $startAt + ($plan->custom_seconds ?? 0);
    ...
    public function checkAndResetQuota(UserSubscription $subscription): void
    {
        ...
        if ($resetPeriod === 'never') return;
        $lastReset = $subscription->last_reset_at ?? $subscription->start_at;
        $now = time();
        $shouldReset = false;
        if ($resetPeriod === 'daily' && $now - $lastReset >= 86400) $shouldReset = true;
        if ($resetPeriod === 'monthly' && $now - $lastReset >= 30 * 86400) $shouldReset = true;
        if ($shouldReset) {
