...
function deleteChannel(id){if(!confirm('确定删除？'))return;fetch(`/web-api/channels/${id}`,{method:'DELETE',headers:{'Accept':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}}).then(r=>{if(!r.ok)return apiError(r);return r.json()}).then(()=>loadChannels(page)).catch(err=>alert('删除失败：'+err.message))}
