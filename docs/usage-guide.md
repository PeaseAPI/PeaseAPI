...
> [00 安装到初始化](guides/00-install-to-init.md) → [01 添加渠道·模型·Key](guides/01-setup-channels-models-keys.md) → [02 Coding 池从零到上线](guides/02-setup-coding-pool.md) → [03 客户·令牌·分组](guides/03-setup-customers-tokens.md)。本篇作为功能全景与细节参考。
...
遇到任何问题都可以在站内提交工单，与管理员一对一沟通，全程留痕可追踪。
...
2. 填写工单表单：
    - **问题标题**：5-100 字，概括问题；
    - **问题分类**：综合问题 / 计费账单 / 技术故障 / 功能建议；
    - **优先级**：低 / 普通 / 高；
    - **问题描述**：5-2000 字，建议写明现象、发生时间、使用的令牌与模型。
3. 提交后自动跳转工单详情页，可随时追加回复（追回）补充信息。
4. 状态流转：`待处理 → 已回复 →（用户追回 → 待处理）→ 已关闭`；问题解决后可点击「关闭工单」。

### 管理侧：处理工单

1. 管理后台侧边栏「**工单管理**」，红点徽标为待处理数（新单 + 用户追回）。
2. 列表按 `待处理 / 已回复 / 已关闭 / 全部` 筛选，展示提交人、分类、优先级与最近更新。
3. 详情页可回复用户、关闭工单，误关可「重开」回到待处理队列。
...
| 待处理 | 新提交的工单，等待客服处理 |
| 已回复 | 客服已回复，等待用户确认 |
| 用户追回 | 用户补充信息/追问，回到待处理队列 |
| 已关闭 | 问题解决或双方确认结束；管理员可重开 |
...
> 💬 如有更多问题，请登录后前往控制台「工单支持」在线提交工单，我们会尽快处理。
