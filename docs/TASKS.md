...
- [ ] P7-5 前端：vendors-tab 加币种选择 + 汇率维护页 ✅ 2026-09-12（十六）——vendors-tab 表单/表格加 `currency`（7 币种 select + 币种列）；新增「汇率」tab（rates CRUD 表格 + hints 生效值提示：CNY 基准/USD Option 兜底）、「厂商活动」tab（promotions 全字段 CRUD + state 徽标 + 倒计时 + 含停用过期开关，复用 vendors 列表供厂商代码 datalist）；typecheck ✓ oxlint 0 warning ✓ build ✓。**待做**：档位/介绍页展示官方原币价 + 折算价双列（介绍页倒计时徽标，用户侧需后端补公开只读 promotions 端点）；coding-plan 页 i18n（现状全中文硬编码，统一改造另行处理）
