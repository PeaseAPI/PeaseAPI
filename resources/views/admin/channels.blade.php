...
<div id="channelModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form id="channelForm" class="p-6 space-y-4">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">类型</label><select id="formType" class="w-full px-4 py-2.5 border rounded-lg text-sm"><option value="1">OpenAI</option><option value="14">Anthropic</option><option value="3">Azure</option><option value="11">Google</option><option value="25">Groq</option><option value="15">百度</option><option value="17">阿里</option><option value="18">讯飞</option><option value="24">Deepseek</option></select></div>
...
@push('scripts')
<script>
const typeNames={1:'OpenAI',14:'Anthropic',3:'Azure',11:'Google',25:'Groq',15:'百度',17:'阿里',18:'讯飞',24:'Deepseek'};
let page=1;
function loadChannels(p=1){page=p;fetch(`/web-api/channels?page=${p}&per_page=10`).then(r=>r.json()).then(d=>{const t=document.getElementById('channelTable');if(d.data&&d.data.length){t.innerHTML=d.data.map(c=>`<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="px-6 py-3">${c.id}</td><td class="px-6 py-3 font-medium">${c.name}</td><td class="px-6 py-3">${typeNames[c.type]||c.type}</td><td class="px-6 py-3">${c.priority||0}</td><td class="px-6 py-3"><span class="px-2 py-1 text-xs rounded-full ${c.status===1?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}">${c.status===1?'启用':'禁用'}</span></td><td class="px-6 py-3"><button onclick="editChannel(${c.id})" class="text-blue-600 hover:text-blue-800 mr-2">编辑</button><button onclick="deleteChannel(${c.id})" class="text-red-600 hover:text-red-800">删除</button></td></tr>`).join('')}else{t.innerHTML='<tr><td colspan="6" class="px-6 py-8 text-center text-gray-500">暂无渠道</td></tr>'}document.getElementById('pageInfo').textContent=`共 ${d.total||0} 条`})}
...
function editChannel(id){fetch(`/web-api/channels/${id}`).then(r=>r.json()).then(c=>{document.getElementById('modalTitle').textContent='编辑渠道';document.getElementById('channelId').value=c.id;document.getElementById('formName').value=c.name;document.getElementById('formType').value=c.type;document.getElementById('formBaseUrl').value=c.base_url||'';document.getElementById('formKey').value=c.key||'';document.getElementById('formModels').value=c.models||'';document.getElementById('formPriority').value=c.priority||0;document.getElementById('formWeight').value=c.weight||1;document.getElementById('formStatus').value=c.status;document.getElementById('channelModal').classList.remove('hidden')})}
function closeModal(){document.getElementById('channelModal').classList.add('hidden')}
function deleteChannel(id){if(!confirm('确定删除？'))return;fetch(`/web-api/channels/${id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'}}).then(r=>r.json()).then(()=>loadChannels(page))}
document.getElementById('channelForm').onsubmit=function(e){e.preventDefault();const id=document.getElementById('channelId').value;const data={name:document.getElementById('formName').value,type:document.getElementById('formType').value,base_url:document.getElementById('formBaseUrl').value,key:document.getElementById('formKey').value,models:document.getElementById('formModels').value,priority:document.getElementById('formPriority').value,weight:document.getElementById('formWeight').value,status:document.getElementById('formStatus').value};const url=id?`/web-api/channels/${id}`:'/web-api/channels';const method=id?'PUT':'POST';fetch(url,{method,headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Content-Type':'application/json'},body:JSON.stringify(data)}).then(r=>r.json()).then(()=>{closeModal();loadChannels(page)})};
loadChannels();
</script>
@endpush