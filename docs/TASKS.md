...
| QA-10 | 中(功能) | Relay/Volcengine | ①`VolcengineAdapter` 是死代码（selectAdapter 把 38/56 归入 openAITypes→OpenAIAdapter），且其默认 base `/api/v3` 与 OpenAIAdapter 固定拼的 `/v1/chat/completions` 相加仍打不通 ark（会变成 /api/v3/v1/...）；②还会给模型名强加 `ark-` 前缀（doubao-seed-evolving 等会请求失败）；③OpenAI 协议上游 base_url 无法适配带版本路径网关（ark coding /api/coding/v3 等固定拼 /v1/...） | 根因确认为 **QA-15**（类型数组存枚举实例，与 int 比较恒 false——所谓"归入 openAITypes"从未生效，一切渠道都落 default OpenAIAdapter）：修复后按类型正确分发；ClaudeAdapter::formatRequest 同时去掉硬编码 api.anthropic.com 改走 getUpstreamUrl('/v1/messages')（尊重渠道 base_url/Coding Plan 覆盖，ark coding /api/coding/v1/messages 实测打通）。VolcengineAdapter 死代码本体留待重构（现 OpenAI 兼容网关经 OpenAIAdapter/ClaudeAdapter 均可用） | ✅ fixed(根因) |
