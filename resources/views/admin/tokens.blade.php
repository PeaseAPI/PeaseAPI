...
<script>
let page=1;
function loadTokens(p=1){page=p;fetch(`/web-api/tokens?page=${p}&per_page=15`, { credentials: 'same-origin' }).then(r=>r.json()).then(d=>{const t=document.getElementById('tokenTable');if(d.data&&d.data.length){t.innerHTML=d.data.map(tk=>`<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="px-6 py-3">${tk.id}</td><td class="px-6 py-3">${tk.username||'-'}</td><td class="px-6 py-3 font-medium">${tk.name}</td><td class="px-6 py-3 font-mono text-xs">${tk.key.substring(0,16)}...</td><td class="px-6 py-3">${tk.unlimited_quota?'∞':(tk.quota_quota||0).toLocaleString()}</td><td class="px-6 py-3">${(tk.quota_used||0).toLocaleString()}</td><td class="px-6 py-3"><span class="px-2 py-1 text-xs rounded-full ${tk.status===1?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}">${tk.status===1?'正常':'禁用'}</span></td><td class="px-6 py-3 text-gray-500">${tk.created_at?new Date(tk.created_at*1000).toLocaleDateString():'-'}</td></tr>`).join('')}else{t.innerHTML='<tr><td colspan="8" class="px-6 py-8 text-center text-gray-500">暂无令牌</td></tr>'}document.getElementById('pageInfo').textContent=`共 ${d.total||0} 条`})}
loadTokens();
</script>
@endpush