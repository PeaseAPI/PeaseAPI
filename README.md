...
| **🆕 站内文档中心** | 统一的站内文档页 `/docs`（快速开始 / 使用指南 / API 参考），配套 `/docs/features` 功能解读页，深浅色自适应、移动端友好 | ⚠️ 原版依赖外部文档 |
| **🆕 工单支持系统** | 用户侧工单提交 / 追回复 / 关闭，管理端会话处理与状态机（待处理 / 已回复 / 追回复 / 已关闭），侧边栏未读红点提醒 | ❌ 原版无 |
...
- **工单支持**：站内工单提交与跟进，客服回复状态实时流转，侧边栏未读红点提醒
...
- **工单管理**：工单统计看板（待处理 / 已回复 / 已关闭计数筛选）、会话处理、状态流转（关闭 / 重开）、快捷回复模板
...
> 📚 **站内文档中心**：除本 README 外，站点内置统一文档页 **`/docs`**（快速开始 / 使用指南 / API 参考）与 **`/docs/features`**（功能解读：定价与订阅、账号池、计费与时段折扣等），深浅色自适应、移动端友好，部署后即可直接访问。
