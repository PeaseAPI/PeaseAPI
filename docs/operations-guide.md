...
- **官方价目已核对预置**（迁移 `000003/000004/000005/000006/000007/000008/000009` + 模板目录 `App\Services\CodingPlanCatalog`，2026-09-11 官方文档核对）：
  ...
  - **分时段折扣预置**（迁移 `2026_09_12_000009`，`coding_plan_model_ratios.time_discounts`）：比率行自带时段窗口数组，引擎按计费时刻自动命中折扣乘到 units（未配置/未命中=原价，折扣强制位于 (0,1)；`end<start` 表跨零点窗口）。已预置官方窗口 4 行：**智谱 glm-5.3/flash**（工作日 14:00–18:00 以外 5 折）、**DeepSeek flash/v4-pro**（高峰=周一至五 9:00–12:00、14:00–18:00，其余减半）；流水 meta 快照记录 `time_discount`/`time_window` 供审计。
