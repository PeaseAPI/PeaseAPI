...
function editUser(id){fetch(`/web-api/users/${id}`, { credentials: 'same-origin' }).then(r=>r.json()).then(u=>{document.getElementById('modalTitle').textContent='编辑用户';document.getElementById('userId').value=u.id;document.getElementById('formUsername').value=u.username;document.getElementById('formEmail').value=u.email||'';document.getElementById('formQuota').value=u.quota||0;document.getElementById('formRole').value=u.role||1;document.getElementById('formStatus').value=u.status;document.getElementById('userModal').classList.remove('hidden')})}
function closeModal(){document.getElementById('userModal').classList.add('hidden')}
function toggleStatus(id,s){fetch(`/web-api/users/${id}`, { credentials: 'same-origin',method:'PUT',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Content-Type':'application/json'},body:JSON.stringify({status:s===1?0:1})}).then(r=>r.json()).then(()=>loadUsers(page))}
