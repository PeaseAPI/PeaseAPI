...
class CodingPlanOfficialSourceService
{
    ...
    protected array $registry = [
        'deepseek' => [
            ...
            'notes' => 'SSR 表格（空闲/高峰双价，转置布局模型在表头行）；V4 Pro 延续声明 2026-09-14 后计费不变；catalog_from_pricing 产 model_catalog（表头行+首列双布局提取，P1-6）',
        ],
        'openai-token' => [
            'label' => 'OpenAI',
        ...
        'xai' => [
            ...
            'proxy' => true,
            'catalog_from_pricing' => true,
            'notes' => '.md 直取（/docs/models 308 → /developers/models）；长上下文分档取 < 200k 首档；Imagine/Voice 按次计价表跳过；catalog_from_pricing 产 model_catalog（token 价表 grok id 去重，2026-09-13 核对无 Retired 段，P1-6）',
        ],
        'zhipu' => [
            'label' => '智谱 BigModel',
        ...
        'moonshot' => [
            ...
            'proxy' => true,
            'catalog_from_pricing' => true,
            'notes' => 'moonshot.cn 301 → platform.kimi.com；llms.txt 索引 + .md 直取；注册国际站 kimi.ai（$，DocTable JSX rows），中文站为 ¥ 不配；列序命中价在前；batch/tools 独立页不配；catalog_from_pricing 产 model_catalog（DocTable rows 首格 kimi id，P1-6）',
        ],
        'volcengine' => [
            'label' => '火山方舟',
        ...
        'minimax' => [
            ...
            'proxy' => true,
            'catalog_from_pricing' => true,
            'notes' => 'llms.txt + .md 直取；注册国际站 minimax.io（$，国内 minimaxi.com 为 ¥ 不配）；Priority Tab（1.5x 条件价）/Legacy Accordion 整段剥离；M3 分档取 ≤512k 首档；划线价取实价；Token Plan 套餐页归 P2-1；catalog_from_pricing 产 model_catalog（token 价表粗体模型 id，P1-6）',
        ],
        'baidu' => [
            'label' => '百度千帆',
