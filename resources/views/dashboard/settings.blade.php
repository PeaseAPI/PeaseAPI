...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    $selectCls = $inputCls . ' appearance-none pr-10';
    $selectStyle = 'style="background-image: url(\'data:image/svg+xml;charset=utf-8,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 20 20%22 fill=%22%23999%22%3E%3Cpath d=%22M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z%22/%3E%3C/svg%3E\'); background-position: right 0.5rem center; background-size: 1.25rem; background-repeat: no-repeat;"';
    @endphp
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form id="securityForm" class="space-y-4">
            ...
            <div>
                ...
                <input type="password" name="current_password" class="{{ $inputCls }}" placeholder="输入当前密码以验证身份">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">新密码</label>
                    <input type="password" name="password" minlength="8" class="{{ $inputCls }}" placeholder="至少 8 位">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">确认新密码</label>
                    <input type="password" name="password_confirmation" minlength="8" class="{{ $inputCls }}" placeholder="再次输入新密码">
                </div>
            </div>
            <div class="flex justify-end">
                <button type="submit" class="px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition font-medium text-sm">更新密码</button>
            </div>
        </form>
    </div>
    ...
    <!-- 偏好设置 -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">偏好设置</h3>
        <form id="prefForm" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">默认语言</label>
                    <select name="language" class="{{ $selectCls }}" {{ $selectStyle }}>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">主题</label>
                    <select name="theme" class="{{ $selectCls }}" {{ $selectStyle }}>
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition font-medium text-sm">保存偏好</button>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900">登录会话</h3>
            <button id="revokeOthersBtn" class="text-sm text-red-600 hover:text-red-700">注销其他会话</button>
        </div>
        <div id="sessionsList" class="space-y-3">
            <p class="text-sm text-gray-500 text-center py-4">加载中...</p>
        </div>
    </div>
</div>
@endsection
...
async function loadSessions() {
    ...
    } catch(e) { document.getElementById('sessionsList').innerHTML = '<p class="text-sm text-gray-500">加载失败</p>'; }
}
